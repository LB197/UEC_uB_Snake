...
XStatus VGA_DRAWRECT_CHAR_Reg_SelfTest(void * baseaddr_p)
{
	...
	for (write_loop_index = 0 ; write_loop_index < 4; write_loop_index++)
	  VGA_DRAWRECT_CHAR_mWriteReg (baseaddr, write_loop_index*4, (write_loop_index+1)*READ_WRITE_MUL_FACTOR);
	for (read_loop_index = 0 ; read_loop_index < 4; read_loop_index++)
	  if ( VGA_DRAWRECT_CHAR_mReadReg (baseaddr, read_loop_index*4) != (read_loop_index+1)*READ_WRITE_MUL_FACTOR){
...
XStatus VGA_DRAWRECT_CHAR_Mem_SelfTest(void * baseaddr_p)
{
	...
	{
	  VGA_DRAWRECT_CHAR_mWriteMemory(baseaddr+4*Index, (0xDEADBEEF % Index));
	}

	for ( Index = 0; Index < 16; Index++ )
	{
	  Mem32Value = VGA_DRAWRECT_CHAR_mReadMemory(baseaddr+4*Index);
